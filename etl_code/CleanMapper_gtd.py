...
for row in reader:
    if len(row)>0:
        if row['iyear']!='iyear' and int(row['iyear'])>=2008 and int(row['iyear'])<=2017:
            print '%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s'%(row['eventid'],row['iyear'],row['imonth'],row['iday'],
            row['country_txt'],row['region_txt'], row['doubtterr'],row['success'], row['suicide'],row['attacktype1_txt'],row['targtype1_txt'],row['natlty1_txt'],row['nkill'])
